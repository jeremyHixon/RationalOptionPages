...
<?php	}
					...
					if ( !empty( $section_params['fields'] ) ) {
						foreach ( $section_params['fields'] as $field_key => $field_params ) {
							...
							if (
								...
								$params['title'] = '<label for="{$params["id"]}">'.__($params['title'],'sigi_core').'</label>';
							}
	...
	protected function build_menu_page( $page_key ) {
		...
		settings_errors();
		?><div class="wrap">
			<h1><?php _e($GLOBALS['title'],'sigi_core'); ?></h1><?php
		...
		switch ( $field['type'] ) {
			case 'checkbox':
				...
				printf(
					...
					$field['value'],																	// value
					!empty( $field['text'] ) ? __($field['text'],'sigi_core') : ''										// text
				);
				break;
			case 'media':
				...
				printf(
					...
					$field['id'],																		// id
					"{$page_key}[{$field['id']}]",														// name
					!empty( $field['placeholder'] ) ? 'placeholder="'.__($field['placeholder'],'sigi_core').'"' : '',		// placeholder
					$field['title_attr'],																// title
					$field['value'],																	// value
					!empty( $attributes ) ? implode( ' ', $attributes ) : '',							// additional attributes
					$upload_button,																		// upload button
					!empty( $field['text'] ) ? '<p class="help">'.__($field['text'],'sigi_core').'</p>' : ''				// text
				);
				break;
			case 'radio':
				echo '<fieldset><legend class="screen-reader-text">' . __($field['title'],'sigi_core') . '</legend>';
				$c = 0;
				foreach ( $field['choices'] as $value => $label ) {
					...
					printf(
						...
						$field['id'],																		// id
						"{$page_key}[{$field['id']}]",														// name
						__($label,'sigi_core'),																				// title
						$value,																				// value
						__($label,'sigi_core'),																				// label
			...
			case 'select':
				...
				printf(
					...
					$field['id'],																		// id
					$field_tag_name,														// name
					__($field['title_attr'],'sigi_core')																// title
				);
			...
			case 'wp_editor':
				...
				echo !empty( $field['text'] ) ? '<p class="help">'.__($field['text'],'sigi_core').'</p>' : '';
				break;
			default:
				printf(
	...
	protected function validate_field( $field, $page_key, $section_key, $field_key, $page, $section ) {
		...
		if ( empty( $field['title'] ) ) {
			$this->submit_error( __('Field parameter "title" is required','sigi_core') );
		}
		...
		// Title attribute
		$field['title_attr'] = empty( $field['title_attr'] ) ? __($field['title'],'sigi_core') : $field['title_attr'];
	...
	protected function validate_page( $page_key, $page_params, $parent_slug = false ) {
		...
		if ( empty( $page_params['page_title'] ) ) {
			$this->submit_error( __('Page parameter "page_title" is required','sigi_core') );
		}
		
		// Menu title
		if ( empty( $page_params['menu_title'] ) ) {
			$page_params['menu_title'] = __($page_params['page_title'],'sigi_core');
		}
	...
	protected function validate_section( $section, $page_key, $section_key, $page ) {
		...
		if ( empty( $section['title'] ) ) {
			$this->submit_error( __('Section parameter "title" is required','sigi_core') );
		}
